...
> [!CAUTION]
> The following script should only be used in testing environments to speed up cluster bootstrapping. It runs continuously in the background, watching for new InstallPlans and can break your rhoai install. It automatically **approves first-time installations only** by checking the operator's Subscription status, ignoring any upgrades. It stops after rhcl has been installed, as it's the last installplan.

```bash
while true; do
  while read -r ns ip csv; do [[ -z "$ns" ]] && continue
    base=$(echo "$csv" | sed -E 's/\.v?[0-9].*//')
    ! oc get csv -n "$ns" 2>/dev/null | grep -qi "$base" && echo "Approving $csv..." && \
    oc patch installplan "$ip" -n "$ns" --type merge -p '{"spec":{"approved":true}}' && \
    [[ "$base" == "rhcl-operator" ]] && echo "Reached RHCL. Stopping." && break 2
  done <<< "$(oc get installplan -A --no-headers 2>/dev/null | awk '$5=="false" {print $1, $2, $3}')"
  sleep 10
done
